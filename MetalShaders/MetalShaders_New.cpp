...
FRHIShaderLibraryRef FMetalDynamicRHI::RHICreateShaderLibrary(EShaderPlatform Platform, FString const& FilePath, FString const& Name)
{
	...
	if ( IFileManager::Get().FileExists(*BinaryShaderFile) == false && !UKismetStringLibrary::Contains(BinaryShaderFile, "Metal/"))
	{
		...
		*BinaryShaderAr << ShaderCode;
		...
		if (Header.Format == PlatformName.GetPlainNameString())
		{
			...
			for (uint32 i = 0; i < Header.NumLibraries; i++)
			{
                ...
                if(UKismetStringLibrary::Contains(MetalLibraryFilePath, TEXT("Metal/")))
                {
                    const auto& Path = FPaths::GetPath(MetalLibraryFilePath);
                    
                    auto& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
                    
                    PlatformFile.CreateDirectoryTree(*FPaths::Combine(Path, TEXT("Lib")));
                    
                    const auto& File = FPaths::GetBaseFilename(MetalLibraryFilePath);
                    
                    const auto LibraryFilePath = FPaths::Combine(Path, TEXT("Lib"), File) + METAL_LIB_EXTENSION;
                    
                    auto FileHandle = PlatformFile.OpenWrite(*LibraryFilePath);
                    ...
                    IFileManager::Get().Copy(*LibraryFilePath, *MetalLibraryFilePath);

                    MetalLibraryFilePath = LibraryFilePath;
                }
